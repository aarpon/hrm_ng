 ...
 *
 *
* @package    propel.generator.hrm.Template.Base
*/
abstract class Template implements ActiveRecordInterface
{
     ...
     * TableMap class name
     */
    const TABLE_MAP = '\\hrm\\Template\\Map\\TemplateTableMap';
    ...
    /**
     * Initializes internal state of hrm\Template\Base\Template object.
     */
     ...
     * @return $this|\hrm\Template\Template The current object (for fluent API support)
     */
    ...
    public function setTemplateId($v)
    {
        ...
        if ($this->template_id !== $v) {
            $this->template_id = $v;
            $this->modifiedColumns[TemplateTableMap::COL_TEMPLATE_ID] = true;
        }
    ...
    public function setClassKey($v)
    {
        ...
        if ($this->class_key !== $v) {
            $this->class_key = $v;
            $this->modifiedColumns[TemplateTableMap::COL_CLASS_KEY] = true;
        }
    ...
    public function hydrate($row, $startcol = 0, $rehydrate = false, $indexType = TableMap::TYPE_NUM)
    {
        try {
            ...
            $col = $row[TableMap::TYPE_NUM == $indexType ? 1 + $startcol : TemplateTableMap::translateFieldName('TemplateId', TableMap::TYPE_PHPNAME, $indexType)];
            $this->template_id = (null !== $col) ? (int) $col : null;

            $col = $row[TableMap::TYPE_NUM == $indexType ? 2 + $startcol : TemplateTableMap::translateFieldName('ClassKey', TableMap::TYPE_PHPNAME, $indexType)];
            ...
            throw new PropelException(sprintf('Error populating %s object', '\\hrm\\Template\\Template'), 0, $e);
        }
    }
    ...
    {
        if ($this->aUser !== null && $this->template_id !== $this->aUser->getId()) {
            $this->aUser = null;
        }
    } // ensureConsistency
    ...
    protected function doInsert(ConnectionInterface $con)
    {
        ...
        if ($this->isColumnModified(TemplateTableMap::COL_TEMPLATE_ID)) {
            $modifiedColumns[':p' . $index++]  = 'template_id';
        }
        if ($this->isColumnModified(TemplateTableMap::COL_CLASS_KEY)) {
     ...
     *                TableMap::TYPE_COLNAME, TableMap::TYPE_FIELDNAME, TableMap::TYPE_NUM.
     *                Defaults to TableMap::TYPE_PHPNAME.
     * @return $this|\hrm\Template\Template
     */
     ...
     * @param string $keyType The type of keys the array uses.
     *
     * @return $this|\hrm\Template\Template The current object, for fluid interface
     */
    ...
    public function buildCriteria()
    {
        ...
        if ($this->isColumnModified(TemplateTableMap::COL_TEMPLATE_ID)) {
            $criteria->add(TemplateTableMap::COL_TEMPLATE_ID, $this->template_id);
        }
        if ($this->isColumnModified(TemplateTableMap::COL_CLASS_KEY)) {
            $criteria->add(TemplateTableMap::COL_CLASS_KEY, $this->class_key);
        }
     ...
     * objects.
     *
     * @param      object $copyObj An object of \hrm\Template\Template (or compatible) type.
     ...
     *
     * @param  boolean $deepCopy Whether to also copy all rows that refer (by fkey) to the current row.
     * @return \hrm\Template\Template Clone of current object.
     * @throws PropelException
     */
    ...
    {
        if ($this->aUser === null && ($this->template_id !== null)) {
            $this->aUser = UserQuery::create()->findPk($this->template_id, $con);
            /* The following can be used additionally to
                guarantee the related object contains a reference
                to this object.  This level of coupling may, however, be
                undesirable since it could result in an only partially populated collection
                in the referenced object.
                $this->aUser->addTemplates($this);
             */
        }
    ...
    {
        if (null !== $this->aUser) {
            $this->aUser->removeTemplate($this);
        }
        $this->id = null;
